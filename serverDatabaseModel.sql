...
CREATE TABLE IF NOT EXISTS `ServerServiceDatabase`.`매장정보` (
  `매장번호` INT NOT NULL AUTO_INCREMENT,
  `주소` VARCHAR(500) NULL,
  `위도` DOUBLE NULL,
  `경도` DOUBLE NULL,
  `이름` VARCHAR(200) NULL,
  ...
  `서비스 가입 날짜` DATE NULL,
  `정보 변경 날짜` DATETIME NULL,
  `매장 개장 시간` TIME(6) NULL,
  `매장 마감 시간` TIME(6) NULL,
...
CREATE TABLE IF NOT EXISTS `ServerServiceDatabase`.`매장공지 정보` (
  `매장번호` INT NOT NULL,
  `공지번호` INT NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`매장번호`),
  UNIQUE INDEX `매장번호_UNIQUE` (`매장번호` ASC),
  UNIQUE INDEX `공지번호_UNIQUE` (`공지번호` ASC),
